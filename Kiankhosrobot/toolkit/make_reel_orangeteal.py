"""
Reusable Instagram Reel builder (spec v2).
Usage: edit CONFIG in a topic driver script, then call build_all(cfg).
Outputs ONLY: <slug>_reel.mp4 (<=4MB, ~20s), <slug>_cover.png
"""
import os, re, math, subprocess, wave
import numpy as np
...
PAL = [((214,106,18),(247,166,54)), ((10,110,124),(38,178,190)),
...
def make_cover(cfg):
    ...
    grad_text(cov, fa, ImageFont.truetype(BLACK, size), (W/2, 560), (255, 236, 170), (212, 148, 20))
    d = ImageDraw.Draw(cov, "RGBA")
    en = cfg["title_en"].upper()
    esz = 58 if len(en) <= 18 else (46 if len(en) <= 24 else 40)
    d.text((W/2, 700), en, font=ImageFont.truetype(BOLD, esz), fill=(255, 255, 255, 235), anchor="mm")
    sub = cfg["subtitle"]
    hw = min(430, 26 + len(sub)*17)
    d.rounded_rectangle([W/2-hw, 790, W/2+hw, 880], 45, fill=(255, 255, 255, 38),
                        outline=(255, 215, 120, 220), width=3)
    d.text((W/2, 835), sub, font=ImageFont.truetype(BOLD, 44), fill=(255, 235, 190), anchor="mm", **KW)
    cov.save("f_cover.png")


def make_slide(idx, cfg):
    ...
    # ZONE 2 image card (no text)
    cw, chh, cy0 = 800, 1000, 330
    cx0 = (W-cw)//2
    ...
    # ZONE 3 text box below card
    n, lh = len(items), 98
    bx0, bx1 = 70, W-70
    by0 = 1390; by1 = by0 + n*lh + 56
    ...
    for k in range(4):
        x = W/2 + (k-1.5)*60
        d.ellipse([x-13, by1+34, x+13, by1+60], fill=(255, 255, 255, 255 if k == idx else 90))
...
def render(cfg, target=20.0):
    ...
    def seg(img, aud, t, out, zin=True, adelay=90, atrim=None, fout=False):
        ...
        cmd = [FF, "-y", "-loop", "1", "-i", img, "-i", aud, "-filter_complex", vf+";"+af,
    ...
    subprocess.run([FF, "-y", "-f", "concat", "-safe", "0", "-i", "gl.txt", "-c", "copy", "g_all.mp4"],
                   check=True, capture_output=True)
    T = dur("g_all.mp4")
    subprocess.run([FF, "-y", "-i", "g_all.mp4", "-stream_loop", "-1", "-i", "bgm.wav", "-filter_complex",
                    f"[1:a]volume=0.17,atrim=0:{T},afade=t=out:st={max(0,T-1.1)}:d=1.1[m];"
                    f"[0:a][m]amix=inputs=2:duration=first:dropout_transition=0,loudnorm=I=-14:TP=-1.5[a]",
                    "-map", "0:v", "-map", "[a]", "-c:v", "copy", "-c:a", "aac", "-b:a", "96k", "mix.mp4"],
                   check=True, capture_output=True)
    for p in (1, 2):
        cmd = [FF, "-y", "-i", "mix.mp4", "-c:v", "libx264", "-b:v", "1250k", "-pass", str(p),
